...
def random_flip_left_right_bbox(image,bboxes,xs,ys,seed=None):
    """Random flip left-right of an image and its bounding boxes.
    """
    def flip_bboxes(bboxes,xs,ys):
        """Flip bounding boxes coordinates.
        """
        bboxes = tf.stack([bboxes[:, 0], 1 - bboxes[:, 3],
                           bboxes[:, 2], 1 - bboxes[:, 1]], axis=-1)
    ...
    with tf.name_scope('random_flip_left_right'):
        ...
        result = control_flow_ops.cond(mirror_cond,
                                       ...
                                       lambda: image)
        # Flip bboxes.
        bboxes,xs,ys = control_flow_ops.cond(mirror_cond,
                                       lambda: flip_bboxes(bboxes,xs,ys),
                                       lambda: (bboxes,xs,ys))
...
def random_rotate90(image, bboxes, xs, ys):
    with tf.name_scope('random_rotate90'):
        elems = tf.convert_to_tensor([1,3],dtype=tf.int32)
        index = random_ops.random_uniform([], 0, 2,dtype=tf.int32)
        k=elems[index]
